...
def create_mock_dataset(n_samples=1000, n_classes=3):
    ...
    for i in range(n_samples):
        ...
        review_words = np.random.choice(word_set + ["book", "kindle", "device", "reading"], n_words)
...
def create_mock_parquet_files(base_dir: Path, n_samples=1000):
    ...
    log.info(f"Созданы мок-файлы: train={len(train_df)}, val={len(val_df)}, test={len(test_df)}")
...
def run_mock_training():
    ...
    mock_env = {
        'FORCE_TRAIN': 'true',  # Принудительно переобучаем
        'OPTUNA_N_TRIALS': '3',  # Только 3 trials для быстроты
        'TFIDF_MAX_FEATURES_MAX': '100',  # Очень мало признаков
        'TFIDF_MAX_FEATURES_MIN': '50',
        'FORCE_SVD_THRESHOLD_MB': '10',  # Низкий порог
        'MEMORY_WARNING_MB': '100',
        'SELECTED_MODELS': 'logreg',  # Только логистическая регрессия
        'OPTUNA_TIMEOUT_SEC': '60',  # 1 минута максимум
    }
    ...
    try:
        log.info(f"Создаю мок-датасет в {temp_dir}")
        ...
        log.info("Запускаю мок-обучение с легкими параметрами...")
        log.info(f"Настройки: trials=3, max_features=100, model=logreg")
        ...
        if mock_model_dir.exists():
            ...
            log.info(f"Создано файлов моделей: {len(model_files)}")
        ...
        log.error(f"Ошибка при мок-обучении: {e}")
        return False
    finally:
